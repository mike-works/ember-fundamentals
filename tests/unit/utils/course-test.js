...
if (require.has('ember-fundamentals/utils/course')) {
  ...
  test('Correct defaults for data', function(assert) {
    ...
    assert.deepEqual(
      course.get('languages'),
      [],
      'languages defaults to empty array'
    );
  });
  ...
  test('Adding and removing tags', function(assert) {
    ...
    assert.equal(
      otherCourse.get('tags').length,
      0,
      'Adding tags to a course doesn not result in it being added to all courses'
    );
  });
  ...
  test('Adding and removing languages', function(assert) {
    ...
    assert.equal(
      otherCourse.get('languages').length,
      0,
      'Adding languages to a course doesn not result in it being added to all courses'
    );
  });

  test('Getting a language from a language ID', function(assert) {
    assert.equal(
      Course.languageName('js'),
      'JavaScript',
      'Course.languageName("js") returns "JavaScript"'
    );
    assert.equal(
      Course.languageName(null),
      'Unknown',
      'Course.languageName() returns "Unknown"'
    );
  });
}
